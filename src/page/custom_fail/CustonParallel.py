...
class CustomParallelProcessor:
    ...
    def run_suite_in_a_thread(func, suite):
        ...
        CustomParallelProcessor.__PARALLELS.update({suite.get_class_object(): {"thread": thread, "tests": []}})
        time.sleep(3)
        thread.start()
        return thread
    ...
    def wait_currently_active_suites_to_finish():
        for suite, info in list(CustomParallelProcessor.__PARALLELS.items()):
            info["thread"].join()
    ...
    def run_test_in_a_thread(func, suite, test, parameter, class_parameter, before_class_error, cancel):
        ...
        if suite.get_class_object() not in CustomParallelProcessor.__PARALLELS.keys():
            CustomParallelProcessor.__PARALLELS.update({suite.get_class_object(): {"thread": thread, "tests": []}})
        CustomParallelProcessor.__PARALLELS[suite.get_class_object()]["tests"].append({"test": test, "thread": thread})
        time.sleep(3)
        thread.start()
        return thread
    ...
    def suite_limit_reached(self):
        active = 0
        for suite, data in CustomParallelProcessor.__PARALLELS.items():
        ...
        for suite, info in list(CustomParallelProcessor.__PARALLELS.items()):
            for test in info["tests"]:
                if test["thread"].is_alive():
                    ...
                    CustomParallelProcessor.__PARALLELS[suite]["tests"].remove(test)
    ...
    def suite_qualifies(self, suite):
        ...
        def _build_reverse_restriction():
            ...
            :return: None
            """
            if restriction not in CustomParallelProcessor.__REVERSE_PARALLEL_RESTRICTIONS:
                CustomParallelProcessor.__REVERSE_PARALLEL_RESTRICTIONS.update({restriction: [suite.get_class_object()]})
            elif suite.get_class_object() not in CustomParallelProcessor.__REVERSE_PARALLEL_RESTRICTIONS[restriction]:
                CustomParallelProcessor.__REVERSE_PARALLEL_RESTRICTIONS[restriction].append(suite.get_class_object())
            else:
        ...
        def _passes_restriction():
            ...
            :return: BOOLEAN
            """
            if CustomParallelProcessor.__PARALLELS.get(restriction, None) is not None:
                if CustomParallelProcessor.__PARALLELS[restriction]["thread"].is_alive():
        ...
        def _passes_reverse_restriction():
            ...
            :return: BOOLEAN
            """
            if suite.get_class_object() in CustomParallelProcessor.__REVERSE_PARALLEL_RESTRICTIONS:
                reverse_suites = CustomParallelProcessor.__REVERSE_PARALLEL_RESTRICTIONS[suite.get_class_object()]
                for reverse_suite in reverse_suites:
                    if CustomParallelProcessor.__PARALLELS.get(reverse_suite, None) is not None:
                        if CustomParallelProcessor.__PARALLELS[reverse_suite]["thread"].is_alive():
    ...
    def test_qualifies(self, test):
        ...
        def _build_reverse_restriction():
            ...
            """
            if restriction not in CustomParallelProcessor.__REVERSE_PARALLEL_RESTRICTIONS:
                CustomParallelProcessor.__REVERSE_PARALLEL_RESTRICTIONS.update({restriction: [test.get_function_object()]})
            elif test.get_function_object() not in CustomParallelProcessor.__REVERSE_PARALLEL_RESTRICTIONS[restriction]:
                CustomParallelProcessor.__REVERSE_PARALLEL_RESTRICTIONS[restriction].append(test.get_function_object())
            else:
                return  # if nothing to add, return - to avoid logging
        ...
        def _passes_restriction():
            ...
            :return: BOOLEAN
            """
            for class_object, suite_mapping in list(CustomParallelProcessor.__PARALLELS.items()):
        ...
        def _passes_reverse_restriction():
            ...
            :return: BOOLEAN
            """
            if test.get_function_object() in CustomParallelProcessor.__REVERSE_PARALLEL_RESTRICTIONS:
                reverse_tests = CustomParallelProcessor.__REVERSE_PARALLEL_RESTRICTIONS[test.get_function_object()]
                for class_object, suite_mapping in list(CustomParallelProcessor.__PARALLELS.items()):
                    for test_mapping in suite_mapping["tests"]:
